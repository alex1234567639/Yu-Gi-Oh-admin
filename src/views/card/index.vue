      ...
      <el-tab-pane :label="$t('card.list')" name="card_list">
        ...
        <div class="filter-container">
          ...
          <div class="filter-item">
            ...
            <el-select v-model="listQuery.filter.type" :placeholder="$t('card.type')" clearable class="select">
              <el-option v-for="item in ygoOptions.type" :key="item" :label="item" :value="item" />
            </el-select>
          </div>
          <!-- 星數 -->
          <div class="filter-item">
            <div class="title">{{ `${$t('card.star')}:` }}</div>
            <el-select v-model="listQuery.filter.star" :placeholder="$t('card.star')" clearable class="select">
              <el-option v-for="item in ygoOptions.star" :key="item" :label="item" :value="item" />
            </el-select>
          </div>
          <!-- 屬性 -->
          <div class="filter-item">
            <div class="title">{{ `${$t('card.attribute')}:` }}</div>
            <el-select v-model="listQuery.filter.attribute" :placeholder="$t('card.attribute')" clearable class="select">
              <el-option v-for="item in ygoOptions.attribute" :key="item" :label="item" :value="item" />
            </el-select>
          </div>
          <!-- 種族 -->
          <div class="filter-item">
            <div class="title">{{ `${$t('card.race')}:` }}</div>
            <el-select v-model="listQuery.filter.race" :placeholder="$t('card.race')" clearable class="select">
              <el-option v-for="item in ygoOptions.race" :key="item" :label="item" :value="item" />
            </el-select>
          </div>
          <!-- 卡片密碼 -->
          <div class="filter-item">
        ...
        <div class="filter-container">
          ...
          <!-- 包裝分類 -->
          <div class="filter-item">
            <div class="title">{{ `${$t('card.product_information_type')}:` }}</div>
            <el-select v-model="listQuery.filter.product_information_type" :placeholder="$t('card.product_information_type')" clearable class="select" style="width:270px">
              <el-option v-for="item in packList" :key="item.id" :label="item.name" :value="item.packType" />
            </el-select>
          </div>
          <!-- 稀有度 -->
          <div class="filter-item">
            <div class="title">{{ `${$t('card.rarity')}:` }}</div>
            <el-select v-model="listQuery.filter.rarity" :placeholder="$t('card.rarity')" clearable class="select">
              <el-option v-for="item in ygoOptions.rare" :key="item" :label="item" :value="item" />
            </el-select>
          </div>
        </div>
        ...
        >
          ...
          <el-table-column :label="$t('card.id')" prop="id" align="center" width="150" />
          <el-table-column :label="$t('card.name')" prop="name" align="center" width="250" />
          <el-table-column :label="$t('card.type')" prop="type" align="center" width="100" />
          <el-table-column :label="$t('card.star')" prop="star" align="center" width="80" />
          <el-table-column :label="$t('card.attribute')" prop="attribute" align="center" width="60" />
          <el-table-column :label="$t('card.race')" prop="race" align="center" width="100" />
          <el-table-column :label="$t('card.atk')" prop="atk" align="center" width="80" />
          <el-table-column :label="$t('card.def')" prop="def" align="center" width="80" />
          <el-table-column :label="$t('card.number')" prop="number" align="center" width="100" />
        </el-table>
